...
CG_EP = {
    ...
    "stock_flow":            "/api/index/stock-flow",                               # Stock-to-Flow璅U? ??
...
# OI API ?敺?甈U?怎? HTTP ??Ⅳ?隤方??荔?靘?process_single_symbol 閮箸?嚗?
...
def fetch_coins_price_change() -> List[Dict]:
    """CoinGlass coins-price-change ?蝡舫?嚗oins-markets 憭望????剁?蝝?CoinGlass 璅U?嚗?"""
...
def fetch_bingx_futures_24h_vol() -> Dict[str, float]:
    """
    Plan B ?漱?澆??湛?BingX 瘞貊??? 24h quoteVolume嚗SDT嚗甈U?敺?    ?桐? API call 瘨菔????BingX 銝?撟?車嚗??湔?垢暺? API Key??    ? {base_symbol: vol_usdt_24h}嚗?憒?{"BTC": 2.3e10, "ETH": 5e9}??    憭望???暺??喟征 dict嚗?敶梢銝餅?蝔?    """
    try:
        r = requests.get(
...
def _fetch_bingx_ticker_snapshot(symbol: str, preferred_symbol: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
    敺?BingX swap v2 ticker 銝甈U?敺???啣 + 24h ?漱憿?USDT)??    ? {"price": float, "volume_usd": float or None}嚗仃????None??    """
    clean = symbol.replace("USDT", "").replace("-", "").replace("_", "").upper()
    try_symbols = [preferred_symbol] if preferred_symbol else []
    try_symbols += [f"{clean}-USDT", f"1000{clean}-USDT"]
...
def _fetch_funding_rate_map() -> Dict[str, float]:
    """
    銝甈U?敺?CoinGlass ?券?鞈?鞎餌?嚗???symbol(base) -> 鞎餌???    ?寞?A嚗I??鞎餌?甇瑕嚗?蝎暹?撣??嚗??蜓???穿?
...
def build_report_message_tiered(
    ...
    def calc_sl_tp(
        ...
        ?: (sl_price, tp1_price, tp2_price, sl_pct, warn_pct, sl_basis_label)
          sl_basis_label: 隤芣? SL 靘?嚗MA20 / 蝯?雿?/ 2H雿?/ ?U皞?
        """
...
def fetch_coinglass_coins_markets() -> List[Dict]:
    ...
    def _try_coins_markets() -> List[Dict]:
        """?? CoinGlass coins-markets Top-100嚗? OI ??嚗?        CoinGlass ??pageNum / sortField ?◤ API 敹賜嚗?甈U?怠??喟??100 蝑?
        ????甈⊿?閮剜?摨?蝭???? API ????        """
        seen_syms: set = set()
    ...
    if result_pc:
        ...
        logger.warning("[CoinGlass-First] coins-price-change ?U??單??")
...
def fetch_position_change():
    ...
    if _cb_is_tripped():
        logger.warning(f"[??劾?沘 ?祈憚隞?MAX_WORKERS=1 ?桀銵?璅U???{_cnt}嚗?")
    elif _cb_is_warned():
        logger.warning(f"[??兩?儭 ?祈憚隞?MAX_WORKERS=2 霅行?璅U???{_cnt}嚗?")
    else:
        logger.info(f"[??兩?] 甇?虜璅U?{_cnt}嚗?")

    logger.info("?? 撅勗祠撟??摰嗥?? ?? | 蝝?CoinGlass 璅U? | 1H MTF ?惜瞍???")

    # ????????????????????????????????????????????????????????
    # 瞍? Step 0嚗???????蝝?CoinGlass 璅U?嚗?
    # ????????????????????????????????????????????????????????
    logger.info("?? [??瞍?] Step 0嚗? CoinGlass 璅U?嚗?????漱??K蝺?OI嚗?靘 CoinGlass API")
    ...
    if MAX_WORKERS == 1:
        logger.warning("[??其??其葉] MAX_WORKERS 撌脤???1嚗頛芣?桀銵?靽風璅U?")
    ...
    for item, cat in [(x, "long_open") for x in top_long_open] + [(x, "long_close") for x in top_long_close] + [(x, "short_open") for x in top_short_open] + [(x, "short_close") for x in top_short_close]:
        ...
        # 鞈?鞎餌?嚗oinGlass ?寞活銵剁?蝝?CoinGlass 璅U?嚗????BingX嚗?
        ...
        with _sniper_file_lock():
            if SNIPER_COOLDOWN_FILE.exists() and _gist_data is None:
                ...
                if _gist_data is None:
                    logger.info(f"?瑕???銝??剁??祈憚?U?駁??? {_cooldown_path_abs}")
    except Exception as e:
        history = []
...
def run_liquidity_radar_once():
    ...
    if not events:
        logger.info("?祆活???U馳蝔桅??唳扔蝡舐???瑼鳴????芷???蝣箄?嚗SI<20 / ?耦??")
        return
...
def _coinglass_simple_get(path: str, params: Optional[Dict] = None) -> Optional[Dict]:
    """蝪U???GET嚗蜓閬策 Altseason / RSI ???格活?亥岷??"""
    if not CG_API_KEY:
        logger.error("CG_API_KEY ?芾身摰??⊥??澆 CoinGlass API")
        return None
    url = f"{CG_API_BASE}{path}"
...
def fetch_hyperliquid_smart_money_score(symbol: str) -> Dict[str, Any]:
    """???閰?嚗??HL ??撣?+ ?Ｗ????嚗?瑟??靽U???    閰??摩嚗?      - HL 憭征瘥?> 1.2嚗??凋蜓撠?+ ??Ｗ?雿??????唳??Ｗ?憭?score > 0
