...
def fetch_oi_change_30m(symbol: str) -> Optional[float]:
    ...
    with _oi_rate_limit_lock:
        ...
        wait_time = 0.7 + random.uniform(0.1, 0.2)
...
# 抄底/摸頭 30m 門檻（微調：放寬讓更多歸類為抄底/摸頭，利於出鑽石）
PRICE_DIP_MAX = 4.0    # 抄底：30m 漲幅 ≤ 4% 才算低位（原 3%）
PRICE_TOP_MIN = -4.0   # 摸頭：30m 跌幅 ≥ -4% 才算高位（原 -3%）

# 24H 趨勢門檻（保守山寨）：12% 以上才當假抄底/假摸頭，適應日波動
TREND_24H_THRESHOLD = 12.0

# 持倉異常策略（已微調：增加 5 星/鑽石出現率）
# ┌────────┬────────────────────────────┬─────────────┬────────────────────────────────────────┐
# │ 訊號   │ 門檻                        │ 倉位比例    │ 說明                                   │
# ├────────┼────────────────────────────┼─────────────┼────────────────────────────────────────┤
# │ 💎鑽石 │ 摸頭/抄底+5星+OI≥3.6%+量≥10M│ 滿倉 7%     │ 24h 成交量≥10M + 鯨魚有數據 + RSI 輔助   │
# │ ⭐5星  │ OI≥3.6%+費率/方向           │ 標準倉 5%   │ 持倉異常明顯                             │
# │ ⭐4星  │ OI≥2.9%+方向                │ 減半倉 2.5% │ 2.9%≤|OI|<3.6%                           │
# └────────┴────────────────────────────┴─────────────┴────────────────────────────────────────┘
# 價格位階：抄底 30m≤4%；摸頭 30m≥-4%。24h 假訊號門檻 12%。主流幣 OI≥5.5% 才進榜。

# 鑽石 RSI 輔助（微調：略放寬以增加鑽石）摸頭≥55 / 抄底≤45，無 RSI 不擋
RSI_FILTER_TOP_MIN = 55   # 摸頭 RSI 原 60
RSI_FILTER_DIP_MAX = 45   # 抄底 RSI 原 40
RSI_FILTER_BREAKOUT_LONG_MIN = 45   # v3.0 追漲時 RSI 不低於 45
RSI_FILTER_BREAKOUT_SHORT_MAX = 55  # v3.0 追跌時 RSI 不高於 55
# 鑽石級 24h 成交量門檻（微調：10M 即給鑽石，原 15M）
VOLUME_ELITE_MIN_USD = 10_000_000
